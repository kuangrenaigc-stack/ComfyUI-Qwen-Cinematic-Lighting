...
def _apply_structure_lock(
    ...
    """Transfer only the low-frequency illumination delta from the Flux proposal back to the source.

    The protected output never contains pixels regenerated by Flux. Only a clamped
    per-pixel exposure ratio (and optionally a low-frequency color cast) is applied
    to the original image, so subject identity, geometry, textures, edges and text
    remain untouched even if Flux re-imagined the scene.
    """
    ...
    strength = max(0.0, min(float(lighting_strength), 1.0))
    exposure_stops = exposure_stops.clamp(-max_stops, max_stops) * strength
...
def _prepare_reference_conditioning(vae, source_image: torch.Tensor, positive_conditioning, negative_conditioning):
    """Attach the source image as a Flux reference latent on the positive branch only.

    Mirroring the stock ``ReferenceLatent`` node, the reference is only attached to
    the positive conditioning. Adding it to negative would push CFG to cancel the
    reference signal, weakening content preservation.
    """
    ...
    return positive, negative_conditioning


def _build_lighting_guider(model, positive_conditioning, negative_conditioning, cfg: float):
...
def _generate_flux_proposal(
    ...
    return vae.decode(sampled["samples"]), sampled


class GeminiCinematicLightingNode(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            category="gemini/cinematic-lighting",
            description=(
            ...
            outputs=[
                ...
                io.Latent.Output("sampled_latent", display_name="Sampled Latent (Raw Flux)"),
            ],
        )
    ...
    def execute(cls, **kwargs) -> io.NodeOutput:
        ...
        positive, negative = PromptCompiler().compile(
            config,
        ...
        guider = _build_lighting_guider(
            model,
            positive_conditioning,
            negative_conditioning,
            float(kwargs.get("cfg", 1.0)),
        )
        flux_proposal, sampled_latent = _generate_flux_proposal(
            vae,
            noise,
            guider,
            sampler,
